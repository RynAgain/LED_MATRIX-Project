    ...
    for s in structures:
        ...
        for dx, dy, intensity in LIGHT_MASK:
            ...
            if 0 <= px < DISPLAY_WIDTH and 0 <= py < DISPLAY_HEIGHT:
                ...
                r, g, b = pixels[px, py]
                r = min(255, int(r + r * boost + 40 * (intensity / 255.0) * nf))
                g = min(255, int(g + g * boost * 0.6 + 20 * (intensity / 255.0) * nf))
...
def _apply_lantern_light(pixels, structures, ambient, camera_x):
    ...
    for s in structures:
        ...
        pixels[lx, ly] = (min(255, max(200, 255 + flicker)), min(255, max(160, 200 + flicker)), 80)
        for dx, dy, intensity in LANTERN_MASK:
            px, py = lx + dx, ly + dy
            if 0 <= px < DISPLAY_WIDTH and 0 <= py < DISPLAY_HEIGHT:
                boost = (intensity / 255.0) * 0.5 * nf
                r, g, b = pixels[px, py]
                r = min(255, int(r + r * boost + 20 * (intensity / 255.0) * nf))
                g = min(255, int(g + g * boost * 0.5 + 10 * (intensity / 255.0) * nf))
                b = min(255, int(b + b * boost * 0.2))
                pixels[px, py] = (r, g, b)
    ...
    for s in structures:
        ...
        for dx, dy, intensity in WATCHTOWER_LIGHT_MASK:
            ...
            if 0 <= px < DISPLAY_WIDTH and 0 <= py < DISPLAY_HEIGHT:
                ...
                r, g, b = pixels[px, py]
                r = min(255, int(r + r * boost + 30 * (intensity / 255.0) * nf))
                g = min(255, int(g + g * boost * 0.5 + 15 * (intensity / 255.0) * nf))
                b = min(255, int(b + b * boost * 0.2))
                pixels[px, py] = (r, g, b)
    ...
    for tx, ty in torch_posts:
        ...
        for dx, dy, intensity in TORCH_POST_LIGHT_MASK:
            ...
            if 0 <= px < DISPLAY_WIDTH and 0 <= py < DISPLAY_HEIGHT:
                ...
                r, g, b = pixels[px, py]
                r = min(255, int(r + r * boost + 20 * (intensity / 255.0) * nf))
                g = min(255, int(g + g * boost * 0.4 + 10 * (intensity / 255.0) * nf))
                b = min(255, int(b + b * boost * 0.2))
                pixels[px, py] = (r, g, b)
